...
<div>
    ...
    <div class="inline_player_playlist_main">
        ...
        // fixme если дождаться исчезновения алерта и снова нажать на плей этой же сломанной песни, он не появиться снова
        // fixme клик по алерту должен приводить к его закрытию
        // fixme алерт перекрывает содержание страницы (например эквалайзер), это потому что скрытие нужно делать
        //  с помощью display none а не прозрачностью
        // fixme должен быть курсор дефолтный для алерта, а не как у текста, и текст на алерте не должно быть возможности выбирать
        // fixme таймер авто закрытия скрывает не только алерт, но и эквалайзер
        $song_id = '4';
        $clicks = 3;
