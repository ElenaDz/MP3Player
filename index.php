...
<div>
    ...
    <div class="inline_player_playlist_main">
        ...
        // fixme запуск исправной песни должен приводить к тому, что алерт исчезает
        // fixme если дождаться исчезновения алерта и снова нажать на плей этой же сломанной песни, он не появиться снова ( не отрабатывает событие ERoRR у audio)
        // fixme клик по алерту должен приводить к его закрытию ok
        // fixme алерт перекрывает содержание страницы (например эквалайзер), это потому что скрытие нужно делать ok
        //  с помощью display none а не прозрачностью ok
        // fixme должен быть курсор дефолтный для алерта, а не как у текста, и текст на алерте не должно быть возможности выбирать ok
        // fixme таймер авто закрытия скрывает не только алерт, но и эквалайзер ok
        $song_id = '4';
        $clicks = 3;
